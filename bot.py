...
# base ("jr" | "tony") -> 6-digit section BIN -> { price_usd, lines, listing_notes }
BIN_STOCK: dict[str, dict[str, dict[str, Any]]] = {}
...
def normalize_shop_base(wallet_or_base: str | None) -> str:
    """Shop / stock bucket: JR CC vs Tony CC."""
    w = (wallet_or_base or "").strip().lower()
    if w in ("tony", "tony_base", "t"):
        return "tony"
    return "jr"


def bin_sections_for_base(base: str) -> dict[str, dict[str, Any]]:
    """Mutable map of 6-digit section BIN -> section dict for one CC base."""
    load_bin_stock()
    b = normalize_shop_base(base)
    BIN_STOCK.setdefault("jr", {})
    BIN_STOCK.setdefault("tony", {})
    return BIN_STOCK[b]
    ...
    if BIN_STOCK_PATH.is_file():
        try:
            ...
            if "jr" in raw or "tony" in raw:
                ...
                if migrated_flat:
                    log.info(
                        "Migrated flat bin_stock.json into jr/tony bases "
                        "(legacy bins → JR CC only)"
                    )
                    save_bin_stock()
...
def save_bin_stock() -> None:
    ...
    BIN_STOCK.setdefault("jr", {})
    BIN_STOCK.setdefault("tony", {})
    out: dict[str, Any] = {"jr": {}, "tony": {}}
    for base in ("jr", "tony"):
        for bk, sec in BIN_STOCK[base].items():
            out[base][bk] = {
...
def admin_stock_base_pick_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "JR CC · JR Bucks",
                    callback_data="adm_stock_pick:jr",
                ),
                InlineKeyboardButton(
                    "Tony CC · Tony Bucks",
                    callback_data="adm_stock_pick:tony",
                ),
            ],
        ]
    )
...
def admin_clear_bin_section(section_bin: str, base: str = "jr") -> bool:
    load_bin_stock()
    bk = str(section_bin).strip()[:6]
    sections = bin_sections_for_base(base)
...
def stock_bins_api_payload() -> dict[str, Any]:
    ...
    for base in ("jr", "tony"):
        ...
        for bk in sorted(BIN_STOCK[base].keys()):
            sec = BIN_STOCK[base][bk]
            lines = sec.get("lines") or []
            sections[bk] = {
                "price_usd": float(sec.get("price_usd", 0.0)),
                "lines": len(lines),
            }
        out[base] = {"sections": sections}
    return {"bases": out}
...
async def handle_sendout(request: web.Request) -> web.Response:
    ...
    bot = ptb_app.bot
    load_bin_stock()
    summary_lines: list[str] = ["📤 <b>Stock sendout</b> <i>(BIN by base)</i>", ""]
    full_text_parts: list[str] = []
    total_lines = 0
    BIN_STOCK.setdefault("jr", {})
    BIN_STOCK.setdefault("tony", {})
    for base in ("jr", "tony"):
        base_label = "Tony CC" if base == "tony" else "JR CC"
        for section_bin in sorted(BIN_STOCK[base].keys()):
            sec = BIN_STOCK[base][section_bin]
            lines = list(sec.get("lines") or [])
            price = float(sec.get("price_usd", 0.0))
            n_lines = len(lines)
            total_lines += n_lines
            summary_lines.append(
                f"• <b>{base_label}</b> · section <b>{section_bin}</b> · "
                f"{fmt_usd(price)}/line · {n_lines} line(s)"
            )
            full_text_parts.append(
                f"\n=== base:{base} section:{section_bin} price:{price} ===\n"
            )
...
<ul>
<li><code>GET /api/stock-bins</code> — BIN sections per base (<code>jr</code> / <code>tony</code>) (JSON)</li>
...
def format_my_orders_html(user_id: int, limit: int = 12) -> str:
    ...
    for i, o in enumerate(recent, 1):
        ...
        wlab = shop_wallet_label(str(o.get("wallet") or "jr"))
        ...
        parts.append(
            f"\n<b>{i}.</b> {html.escape(at_s)} UTC · {html.escape(wlab)}\n"
...
# TONY BASE Cash App: user sends $cashtag after choosing this method (manual admin approval).
...
class _AwaitingCashtagMessageFilter(filters.MessageFilter):
    """After user chose Cash App (Tony base), next text = their $cashtag for the claim."""
...
class _MainReplyMenuTextFilter(filters.MessageFilter):
    """Exact labels from main_reply_keyboard (Credits: row matches prefix)."""
...
def admin_portal_menu_html() -> str:
    ...
    return (
        ...
        "<b>Upload cards</b> = add stock (JR or Tony base). "
        "Use <b>Stock overview</b> to see counts by BIN."
    )
...
def admin_bin_notebook_html() -> str:
    ...
    BIN_STOCK.setdefault("tony", {})
    lines_out: list[str] = [
        "🗄️ <b>BIN notebook</b> <i>(stock by base)</i>\n",
    ]
    for base, label in (("jr", "JR CC"), ("tony", "Tony CC")):
        secs = BIN_STOCK.get(base) or {}
        if not secs:
            lines_out.append(f"\n<b>{label}</b>: <i>no sections</i>")
            continue
        lines_out.append(f"\n<b>{label}</b>")
        for bk in sorted(secs.keys()):
            sec = secs[bk]
            n = len(sec.get("lines") or [])
            price = float(sec.get("price_usd", 0.0))
            lines_out.append(
                f"· <code>{html.escape(bk)}</code> — {fmt_usd(price)}/line · "
                f"<b>{n}</b> line(s)"
            )
    lines_out.append(
...
def section_catalog_html_title(section_bin: str, wallet_key: str = "jr") -> str:
    load_bin_stock()
    sec = bin_sections_for_base(wallet_key).get(section_bin) or {}
    price = float(sec.get("price_usd", 0.0))
...
def buy_menu_keyboard(wallet_key: str = "jr") -> InlineKeyboardMarkup:
    """One row per BIN section for the selected CC base (JR vs Tony)."""
    load_bin_stock()
    wk = normalize_shop_base(wallet_key)
    rows: list[list[InlineKeyboardButton]] = []
    sections = bin_sections_for_base(wk)
...
def section_catalog_text_and_keyboard(
    ...
    u = ensure_user(user_id)
    section_bin = section_bin.strip()[:6]
    wk_norm = normalize_shop_base(wallet_key)
    sec = bin_sections_for_base(wk_norm).get(section_bin)
    head = section_catalog_html_title(section_bin, wallet_key)
...
async def handle_buy_product(
    ...
    section_bin = section_bin.strip()[:6]
    load_bin_stock()
    wk = normalize_shop_base(context.user_data.get("purchase_wallet"))
    sec = bin_sections_for_base(wk).get(section_bin)
    if not sec:
    ...
    sections_map = bin_sections_for_base(wk)
    if not lines_flat:
    ...
    save_bin_stock()
    try:
        record_purchase_order(user.id, wk, section_bin, bucket_key, price, line)
    except Exception:
    ...
    await query.message.reply_text(
        f"✅ <b>Delivered</b> · "
        f"{section_catalog_html_title(section_bin, wk)}\n"
...
async def handle_buy_catalog_page(
    ...
    load_bin_stock()
    wk = normalize_shop_base(context.user_data.get("purchase_wallet"))
    if section_bin not in bin_sections_for_base(wk) or page < 0:
        await query.answer("Invalid.", show_alert=True)
        return
...
def pay_method_keyboard(pay_base: str | None = None) -> InlineKeyboardMarkup:
    """BTC/LTC always; Cash App only for TONY BASE (not GetToTheMoneyJR)."""
    rows: list[list[InlineKeyboardButton]] = [
        [
    ...
    if payment_base_key(pay_base) == "tony":
        rows.append(
            [
                InlineKeyboardButton(
                    "💵 Cash App",
    ...
    rows.append([InlineKeyboardButton("⬅️ Back", callback_data="pay_m_back")])
    return InlineKeyboardMarkup(rows)
...
• Minimum: <b>$10</b>
• Pick a <b>base</b> first — each base has its own options.

• <b>GetToTheMoneyJR</b> → <b>JR Bucks</b> (BTC / LTC on-chain)
• <b>TONY BASE</b> → <b>Tony Bucks</b> (BTC / LTC, or <b>Cash App</b> with $cashtag — admin verifies manually)

After approval, the matching balance updates."""
...
async def handle_main_reply_menu(
    ...
    if text == BTN_REPLY_BUY:
        await msg.reply_text(
            "💳 <b>Buy cards</b>\n\n"
            "<b>JR Bucks</b> — GetToTheMoneyJR shop.\n"
            "<b>Tony Bucks</b> — TONY BASE shop.\n\n"
            "Pick a wallet:",
    ...
    if text.startswith("Credits:"):
        ...
        await msg.reply_text(
            ...
            f"<b>JR Bucks:</b> {fmt_usd(jr)}\n"
            f"<b>Tony Bucks:</b> {fmt_usd(tb)}\n"
            f"<b>Total credits:</b> {fmt_usd(jr + tb)}\n\n"
...
def pay_method_text(
    amount: float, base_label: str = "", tony_base: bool = False
) -> str:
    base_line = (
    ...
    if tony_base:
        return (
            ...
            f"{base_line}"
            "• <b>BTC</b> / <b>LTC</b> — on-chain, then submit explorer link as usual.\n"
            "• <b>Cash App</b> — send payment off-bot, then submit your <b>$cashtag</b> here; "
            "an admin approves <b>manually</b>. Typical wait: <b>3–10 min</b> (EST).\n\n"
            "Choose:"
        )
    return (
        "💰 <b>SELECT PAYMENT METHOD</b>\n\n"
...
def cashapp_prompt_html(amount: float, base_label: str) -> str:
    return (
        "💵 <b>Cash App</b> <i>(TONY BASE · Tony Bucks)</i>\n\n"
        ...
        "An admin will verify and approve your top-up <b>manually</b>.\n"
        "Typical approval time: <b>3–10 minutes</b> (EST).\n\n"
        "<i>Not for GetToTheMoneyJR — Tony base only.</i>"
    )
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "adm_stock":
        ...
        context.user_data["admin_stock_step"] = "base"
        await query.message.reply_text(
            "📦 <b>Add stock — step 1/5</b>\n\n"
            "Choose which <b>CC base</b> this inventory is for:\n"
            "• <b>JR CC</b> — shows only in the <b>JR Bucks</b> shop\n"
            "• <b>Tony CC</b> — shows only in the <b>Tony Bucks</b> shop\n\n"
            "<i>One pasted line = one unit sold. /cancel to abort.</i>",
            parse_mode="HTML",
            reply_markup=admin_stock_base_pick_keyboard(),
        )
        return
    ...
    if data.startswith("adm_stock_pick:"):
        if not is_admin(user.id):
            await query.answer("Not authorized.", show_alert=True)
            return
        base = (data.split(":", 1)[1] if ":" in data else "").strip()
        if base not in ("jr", "tony"):
            await query.answer("Invalid base.", show_alert=True)
            return
        await query.answer()
        mark_admin_stock_wizard(user.id)
        context.user_data["stock_flow_base"] = base
        context.user_data["admin_stock_step"] = "bin"
        lbl = (
            "Tony CC (TONY BASE · Tony Bucks)"
            if base == "tony"
            else "JR CC (GetToTheMoneyJR · JR Bucks)"
        )
        await query.message.reply_text(
            f"📦 Base: <b>{lbl}</b>\n\n"
            "<b>Step 2/5</b> — Send the <b>6-digit BIN</b> for this section "
    ...
    if data == "adm_port_upload":
        ...
        await query.message.reply_text(
            ...
            "Tap <b>📦 Add stock</b> below for the guided wizard: "
            "choose <b>JR CC</b> or <b>Tony CC</b> base → BIN → paste lines → price → notes.\n\n"
            "Or send <code>/stock</code> with no args for the same wizard, or "
            "<code>/stock bin …</code> to add one line quickly.\n\n"
            "<i>One line = one sale. Use /cancel to abort the wizard.</i>",
            parse_mode="HTML",
    ...
    if data.startswith("open_sec:"):
        ...
        if len(section_bin) != 6 or section_bin not in bin_sections_for_base(wk):
    ...
    if data == "base_gtjm":
        ...
        await edit_safe(
            query,
            pay_method_text(amt, payment_base_label("gtjm"), tony_base=False),
            pay_method_keyboard("gtjm"),
        )
        return
    ...
    if data == "base_tony":
        ...
        await edit_safe(
            query,
            pay_method_text(amt, payment_base_label("tony"), tony_base=True),
            pay_method_keyboard("tony"),
        )
        return
    ...
    if data == "m_bases":
        await query.message.reply_text(
            "🏦 <b>Payment bases</b>\n\n"
            "<b>GetToTheMoneyJR</b> — <code>BASE_GTJM_BTC</code> / "
            "<code>BASE_GTJM_LTC</code> (falls back to legacy wallet env vars if unset). "
            "Credits <b>JR Bucks</b>. <b>No Cash App</b> on this base.\n\n"
            "<b>TONY BASE</b> — <code>BASE_TONY_BTC</code> / <code>BASE_TONY_LTC</code>, "
            "or <b>Cash App</b> (submit <b>$cashtag</b> after paying — admin approves manually, "
            "typically <b>3–10 min</b> EST). Credits <b>Tony Bucks</b>.\n\n"
            "When you <b>Top Up</b>, pick a base first.",
            parse_mode="HTML",
        )
        return
    ...
    if data in ("m_cart", "m_buy"):
        await query.message.reply_text(
            ...
            "Pick JR or Tony wallet — each shop only shows stock added for that base.",
            reply_markup=shop_wallet_select_keyboard(),
            parse_mode="HTML",
        )
        return
    ...
    if data == "pay_cashapp":
        ...
        if base_k != "tony":
            await query.answer(
                "Cash App is only for TONY BASE (Tony Bucks).",
                show_alert=True,
            )
            return
        await query.answer()
        ...
        await edit_safe(
            query,
            cashapp_prompt_html(amt, base_lbl),
    ...
    if data == "pay_cashapp_cancel":
        ...
        await edit_safe(
            ...
            pay_method_text(
                amt,
                payment_base_label(base_k),
                tony_base=(base_k == "tony"),
            ),
            pay_method_keyboard(context.user_data.get("pay_base")),
        )
        return
...
async def handle_payment_cashtag_message(
    ...
    """Submit Cash App $cashtag for TONY BASE — creates claim for manual admin approval."""
    if not update.message or not update.effective_user:
        return
    user = update.effective_user
    uid = user.id
    ...
    base_k = payment_base_key(context.user_data.get("pay_base"))
    if base_k != "tony" or str(context.user_data.get("pay_coin") or "") != "cashapp":
        clear_awaiting_cashtag(uid)
        return
    ...
    await update.message.reply_text(
        ...
        "An admin will verify your Cash App payment and credit <b>Tony Bucks</b>.\n"
...
async def handle_admin_stock_message(
    ...
    if step == "base":
        low = text.lower()
        if low in ("jr", "j", "gtjm", "gettothemoneyjr"):
            base_k = "jr"
        elif low in ("tony", "t", "tony base", "tony cc"):
            base_k = "tony"
        else:
            await msg.reply_text(
                "Send <code>jr</code> or <code>tony</code>, or tap a button on the "
                "previous message.",
                parse_mode="HTML",
            )
            return
        context.user_data["stock_flow_base"] = base_k
        context.user_data["admin_stock_step"] = "bin"
        lbl = (
            "Tony CC (TONY BASE · Tony Bucks)"
            if base_k == "tony"
    ...
    if step == "bin":
        ...
        await msg.reply_text(
            f"📦 <b>Step 3/5</b> — BIN <code>{html.escape(bk)}</code>\n\n"
    ...
    if step == "lines":
        ...
        await msg.reply_text(
            "📦 <b>Step 4/5</b> — Price per line for this pile\n\n"
    ...
    if step == "price":
        ...
        await msg.reply_text(
            "📦 <b>Step 5/5</b> — Buyer-facing notes\n\n"
    ...
    if step == "notes":
        flow_base = normalize_shop_base(context.user_data.get("stock_flow_base"))
        bin_sec = (context.user_data.get("stock_flow_bin") or "").strip()[:6]
        ...
        load_bin_stock()
        sections_map = bin_sections_for_base(flow_base)
        ...
        save_bin_stock()
        n = len(sec["lines"])
        base_lbl = "Tony CC" if flow_base == "tony" else "JR CC"
        clear_admin_stock_flow(context, user.id)
        await msg.reply_text(
            f"✅ Stock saved · <b>{html.escape(base_lbl)}</b> · section "
            f"<code>{html.escape(bin_sec)}</code>\n"
            f"{fmt_usd(price)} per line · <b>{len(pending)}</b> line(s) added · "
            f"<b>{n}</b> total line(s) in this base’s section.",
            parse_mode="HTML",
        )
        return
...
async def cmd_stock(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(args) < 6:
        await update.message.reply_text(
            "Usage (admin):\n"
            "<code>/stock</code> — guided add (base → BIN → paste → price → notes)\n"
            "<code>/stock bin [jr|tony] BIN $price PAN MM/YY CVV</code> — one line\n\n"
            "<code>jr</code> = JR Bucks shop · <code>tony</code> = Tony Bucks shop. "
            "Omit base to default to <b>jr</b>.\n\n"
            "Examples:\n"
            "<code>/stock bin tony 535585 $8 5355862208760853 06/29 178</code>\n"
    ...
    if args[0].lower() != "bin":
        await update.message.reply_text(
            "Start with <code>bin</code>, then optional <code>jr</code>/<code>tony</code>, "
            "then the 6-digit BIN.",
            parse_mode="HTML",
        )
        return
    idx = 1
    stock_base = "jr"
    if len(args) > idx and args[idx].lower() in ("jr", "tony", "gtjm"):
        stock_base = "tony" if args[idx].lower() == "tony" else "jr"
        idx += 1
    if len(args) < idx + 5:
        await update.message.reply_text(
            "Not enough arguments. Use:\n"
            "<code>/stock bin [jr|tony] BIN $price PAN MM/YY CVV</code>",
            parse_mode="HTML",
        )
        return
    ...
    load_bin_stock()
    flow_base = normalize_shop_base(stock_base)
    sec = bin_sections_for_base(flow_base).setdefault(
        bin_sec,
        {"price_usd": float(price), "lines": [], "listing_notes": ""},
    )
    sec["price_usd"] = float(price)
    ...
    save_bin_stock()
    n = len(sec["lines"])
    bl = "Tony CC" if flow_base == "tony" else "JR CC"
    await update.message.reply_text(
        f"✅ Stock added · <b>{html.escape(bl)}</b> · section "
        f"<code>{html.escape(bin_sec)}</code> · "
